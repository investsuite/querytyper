...
ENV VIRTUAL_ENV /opt/querytyper-env
ENV PATH $VIRTUAL_ENV/bin:$PATH
RUN python -m venv $VIRTUAL_ENV

# Set the working directory.
WORKDIR /workspaces/querytyper/
...
COPY --chown=user:user poetry.lock* pyproject.toml /workspaces/querytyper/
RUN mkdir -p /home/user/.cache/pypoetry/ && mkdir -p /home/user/.config/pypoetry/ && \
    mkdir -p src/querytyper/ && touch src/querytyper/__init__.py && touch README.md
...
COPY --chown=user:user .pre-commit-config.yaml /workspaces/querytyper/
